...
def _render_canonical_typst(report: Mapping[str, Any], *, title: str | None) -> str:
    ...
    lines = [
        ...
        "#set page(margin: (x: 1.65cm, y: 1.35cm))",
        ...
        "",
        "#align(center)[",
        f"  #text(size: 19pt, weight: \"bold\", fill: ink)[{_escape_typst_markup(title)}]",
        "]",
        "#align(center)[",
        "  #text(size: 8pt, fill: muted)[Aggregate-only supplementary dashboard; detailed thresholds and confidence intervals remain in the pairwise reports.]",
        "]",
        "",
        "== Scope and T1 gate",
        "",
        ...
        f"    #strong[Course] #text(fill: muted)[{_escape_typst_markup(str(course['course_id']))}]",
        "    #linebreak()",
        f"    #strong[Assessment] #text(fill: muted)[{_escape_typst_markup(str(course['assessment_id']))}]",
        "    #linebreak()",
