...
| TASK5 | GitHub Actions CI quality gate | Project infrastructure | `running` | Branch `agent/task5-github-actions-ci` adds guarded core/Physics suites plus explicit privacy, held-out, sealed-test, and research-provenance checks | Pass local equivalents, open the bilingual PR, then require the exact contexts on `main` | Bilingual settlement draft prepared; PR link pending |
