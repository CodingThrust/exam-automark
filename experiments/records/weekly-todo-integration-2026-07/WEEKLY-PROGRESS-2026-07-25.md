...
| TASK4 | sci-brain survey explanation | Previous-week carry-over | `running` | Teaching/options PR #33 merged; branch `agent/task4-survey-provenance` adds an explicit legacy gap, future provenance requirements, citation-scope audit, 217 passing core tests, and a bilingual settlement draft | Open and merge the provenance PR, add its URL to the settlement, then post the prepared settlement to Zulip | Bilingual settlement draft ready, not posted |
