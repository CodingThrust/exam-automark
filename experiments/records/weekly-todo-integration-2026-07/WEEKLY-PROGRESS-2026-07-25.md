...
| TASK4 | sci-brain survey explanation | Previous-week carry-over | `completed` | Teaching/options PR #33 and provenance PR #34 merged; 217 core tests passed; bilingual settlement is ready | Post the prepared settlement to Zulip when a posting channel is available | Bilingual settlement ready, not posted |
| TASK5 | GitHub Actions CI quality gate | Project infrastructure | `completed` | PR #35 merged as `f51c5e0`; the post-merge `main` push run passed all three required Ubuntu checks; approvals remain 0 | Keep the stable check contexts required while later TASKs extend coverage deliberately | Bilingual settlement ready, not posted |
| TASK8 | Candidate-v3.2 typical-case error book and defect audit | This-week TODO3; advisor accuracy priority | `completed` | Complete private 33-case error book and bilingual diagnosis; public-safe score/cause aggregates and bilingual reports; privacy gates; 233 core + 85 physics tests passed | Ask the course owner to adjudicate Q8 epsilon, Q6 full-credit, and Q10 score-grid conflicts; carry the resulting grounded labels into TASK6/TASK9 | Bilingual settlement ready, not posted |
| TASK6 | Objective error taxonomy and confidence calibration | This-week TODO4 + TODO5 | `pending` | Existing metrics contain accuracy and some confidence fields; TASK8 will supply the grounded cases | Freeze the taxonomy and calibration metrics from TASK8 cases rather than designing them abstractly | Pending |
| TASK9 | Evidence-guided grading-skill refinement | This-week TODO6; advisor accuracy priority | `pending` | Advisor execution/submission skill is complete; candidate edits must wait for TASK8/TASK6 evidence | Change one evidenced failure mechanism per candidate and require a dev accuracy improvement without severe-error regression | Pending |
| TASK7 | Automated Codex CLI versus Claude Code multimodal comparison | This-week TODO2 | `pending` | User accepted option C; Claude route is in the advisor skill, but matched Codex multimodal evidence and a unified lineage contract are missing | Run matched transcript and direct-multimodal arms on the grounded typical-case subset after the first accuracy iteration | Pending |
...
| TASK11 | Optional project-owned skill optimization loop | Previous-week TODO8 carry-over; advisor optional innovation | `pending` | Deterministic dry-run MVP only; no proven manual accuracy-improvement loop yet | Automate TASK8-to-TASK9 only after one manual, interpretable dev improvement succeeds | Pending |
