...
"""Freeze a deterministic anonymous development/held-out split without a model."""
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Freeze a deterministic anonymous development/held-out split using "
            "SHA-256(seed + '|' + student_id). This command never reads submissions "
            "or calls a model."
        )
    )
    parser.add_argument("--course", type=Path, required=True)
    parser.add_argument("--seed", required=True)
    parser.add_argument("--heldout-count", type=_positive_int, required=True)
    parser.add_argument("--output", type=Path, required=True)
    parser.add_argument("--development-students-file", type=Path, required=True)
    parser.add_argument("--heldout-students-file", type=Path, required=True)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        result = freeze_split(
            ...
            development_students_file=args.development_students_file,
            heldout_students_file=args.heldout_students_file,
        )
