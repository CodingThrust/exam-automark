...
<p class="hint">Review the already-redacted anonymous page. Approve only when identity is hidden, human grading evidence is hidden, and required student work remains visible.</p>
...
async function load(keep=true){const old=keep&&state?state.pages[index]?.anonymous_id+'|'+state.pages[index]?.source_page:null;state=await api('/api/state');$('#summary').textContent=`${state.summary.fully_approved}/${state.summary.page_count} fully approved; ${state.summary.needs_correction} need correction`;const select=$('#page');select.replaceChildren();state.pages.forEach((p,i)=>{const o=document.createElement('option');o.value=i;o.textContent=`${p.anonymous_id} / source page ${p.source_page}`;select.append(o)});if(old){const i=state.pages.findIndex(p=>p.anonymous_id+'|'+p.source_page===old);if(i>=0)index=i}render();}
...
async function approve(){const reviewer=$('#reviewer').value.trim();if(!reviewer){alert('Enter your reviewer name or initials first.');return}if(!confirm('Approve identity privacy, blindness, and content preservation for this page?'))return;const p=page();try{await api('/api/approve-all','POST',{anonymous_id:p.anonymous_id,source_page:p.source_page,reviewer,reviewed_at:now()});$('#message').textContent='All three approvals saved.';await load()}catch(e){$('#message').textContent=e.message}}
async function reject(){const reviewer=$('#reviewer').value.trim();if(!reviewer){alert('Enter your reviewer name or initials first.');return}const note=prompt('What must be corrected (identity leak, remaining grading mark, or hidden content)?')||'';if(!note.trim()){alert('A correction note is required.');return}const p=page();try{await api('/api/reject','POST',{anonymous_id:p.anonymous_id,source_page:p.source_page,reviewer,reviewed_at:now(),note});$('#message').textContent='Correction requirement saved.';await load()}catch(e){$('#message').textContent=e.message}}
