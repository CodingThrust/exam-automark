...
function render(){ draft=null;draftStart=null;const p=page(); $('#page').value=index; $('#title').textContent=`${p.anonymous_id}, source page ${p.source_page}`; const candidates=$('#candidates');candidates.replaceChildren(...p.candidates.map(candidateCard)); updateManualSummary(); image=new Image(); image.onload=()=>{canvas.width=image.naturalWidth;canvas.height=image.naturalHeight;draw()}; image.src=protectedPath('/images/'+encodeURIComponent(p.image_path)); }
