...
<p class="hint">Local only. Candidate boxes are suggestions, not masks. Inspect the full page, draw any missed grading evidence, then complete the page sweep. Completing a page moves to the next incomplete sweep.</p>
<p id="binding"></p>
...
function sweepComplete(p){return p.sweep?.sweep_status==='completed'}
function nextIncompleteSweepIndex(after){for(let offset=1;offset<=state.pages.length;offset++){const i=(after+offset)%state.pages.length;if(!sweepComplete(state.pages[i]))return i}return after}
async function load(keep=true,focusIncompleteAfter=null){ const old=keep&&state?state.pages[index]?.anonymous_id+'|'+state.pages[index]?.source_page:null; state=await api('/api/state'); const binding=state.binding||{}; $('#binding').textContent=binding.warning||'Input binding verified: layout, candidate pack, and rendered artifact manifest belong together.'; $('#binding').className=binding.warning?'warning':'ok'; const remaining=state.summary.page_count-state.summary.sweeps_completed; $('#summary').textContent=`${state.summary.sweeps_completed}/${state.summary.page_count} page sweeps complete; ${remaining} remaining; ${state.summary.candidates_resolved}/${state.summary.candidate_count} candidates resolved`; const selector=$('#page'); selector.replaceChildren(); state.pages.forEach((p,i)=>{const o=document.createElement('option');o.value=i;o.textContent=`${p.anonymous_id} / source page ${p.source_page}`;selector.append(o)}); if(focusIncompleteAfter!==null){index=nextIncompleteSweepIndex(focusIncompleteAfter)}else if(old){const i=state.pages.findIndex(p=>p.anonymous_id+'|'+p.source_page===old);if(i>=0)index=i} render(); }
...
async function saveSweep(status){ const reviewer=$('#reviewer').value.trim(); if(status==='completed'&&!reviewer){alert('Enter your reviewer name or initials first.');return} const p=page(), completedIndex=index, pending=unsavedRects(), added=[...rects(p.sweep?.added_rectangles),...pending]; try{await api('/api/sweep','POST',{anonymous_id:p.anonymous_id,source_page:p.source_page,sweep_status:status,reviewer,reviewed_at:now(),added_rectangles:added,notes:''});setUnsaved([]);await load(status!=='completed',status==='completed'?completedIndex:null);$('#message').textContent=status==='completed'?'Page sweep saved. Moved to the next incomplete page.':'Page sweep saved.'}catch(e){$('#message').textContent=e.message} }
