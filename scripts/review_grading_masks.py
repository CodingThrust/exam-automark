...
<div id="main"><div><canvas id="canvas"></canvas><p class="hint">Drag on the page to add a manual grading-mark rectangle. It is saved with the page sweep.</p></div>
<div id="panel"><h2 id="title"></h2><div id="candidates"></div><hr><h3>Full-page sweep</h3><p class="hint">This is mandatory even when there are zero candidates. It must catch grayscale marks the detector cannot distinguish from student writing.</p><div id="manual"></div><button id="complete">Save and complete sweep</button><button id="pending">Save as pending</button><p id="message"></p></div></div>
<script>
let state, index=0, image, canvas=document.querySelector('#canvas'), ctx=canvas.getContext('2d'), draft=null, draftStart=null;
