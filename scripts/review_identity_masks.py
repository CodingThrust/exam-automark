...
<style>body{font-family:system-ui,sans-serif;margin:16px;background:#f7f7f8;color:#1f2937}button,input,textarea{font:inherit;margin:4px}.bar{display:flex;flex-wrap:wrap;gap:8px;align-items:center}.canvas{position:relative;display:inline-block;max-width:100%;margin-top:12px}.canvas img{display:block;max-width:min(100%,1000px);max-height:72vh;cursor:crosshair;touch-action:none;user-select:none;-webkit-user-drag:none}.mask{position:absolute;border:2px solid #dc2626;background:#dc262633;pointer-events:none}.proposal{position:absolute;border:2px dashed #6b7280;background:#6b72801a;pointer-events:none}.note{max-width:1000px;color:#4b5563}</style></head><body>
<h1>Private identity-mask review</h1><p class="note">Local only. Grey dashed boxes are untrusted geometry suggestions only: they are never saved and may cover question content. Red boxes are the only masks that will be saved. Clear red boxes, then drag precisely over names/student IDs only. If a page has no personal information, leave red boxes empty and explicitly approve it as no-identity.</p>
<div class="bar"><button id="prev">Previous</button><button id="next">Next</button><strong id="label"></strong><span id="summary"></span></div>
<div class="bar"><label>Reviewer <input id="reviewer" required></label><button id="suggest">Copy grey suggestions to red masks</button><button id="clear">Clear red masks</button><button id="undo">Undo last red mask</button><button id="approve">Approve with red mask</button><button id="noidentity">Approve no identity</button><button id="correct">Save needs-correction</button></div>
...
function page(){return state.pages[index]};function addBox(r,kind){const d=document.createElement('div');d.className=kind;d.style.left=(r.left*100)+'%';d.style.top=(r.top*100)+'%';d.style.width=((r.right-r.left)*100)+'%';d.style.height=((r.bottom-r.top)*100)+'%';$('canvas').appendChild(d)}function draw(){const box=$('canvas');box.querySelectorAll('.mask,.proposal').forEach(x=>x.remove());for(const r of page().proposed_rectangles)addBox(r,'proposal');for(const r of draft)addBox(r,'mask')}
function render(){const p=page();$('label').textContent=`${index+1}/${state.pages.length} — ${p.anonymous_id}, local page ${p.local_page} (${p.review_status})`;$('notes').value=p.notes||'';draft=JSON.parse(JSON.stringify(p.approved_rectangles));const img=$('image');img.onload=draw;img.src='/images/'+p.source_page+'?token='+encodeURIComponent(token);}
function point(e){const r=$('image').getBoundingClientRect();return{x:Math.max(0,Math.min(1,(e.clientX-r.left)/r.width)),y:Math.max(0,Math.min(1,(e.clientY-r.top)/r.height))}}let start=null;$('image').addEventListener('pointerdown',e=>{e.preventDefault();start=point(e)});$('image').addEventListener('pointerup',e=>{e.preventDefault();if(!start)return;const end=point(e);if(Math.abs(end.x-start.x)>.005&&Math.abs(end.y-start.y)>.005){draft.push({left:Math.min(start.x,end.x),top:Math.min(start.y,end.y),right:Math.max(start.x,end.x),bottom:Math.max(start.y,end.y)});draw()}start=null});$('image').addEventListener('pointercancel',()=>{start=null});
