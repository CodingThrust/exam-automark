...
<body><h1>Private page-scope review</h1><p class="hint">This is not grading. Each group below has a nonstandard page count. Inspect the already-anonymized pages only to decide whether they all belong to the same student's submission. Choose “all pages belong” when they do; this records page ownership only and does not make the cohort or any model run ready. Choose “requires correction” for a missing page, duplicate, separator, unrelated page, or extra continuation that needs a revised layout/scope. Nothing is deleted here.</p><p id="binding" class="ok">Input binding verified. No model is called.</p><div id="top"><label>Reviewer <input id="reviewer" placeholder="name or initials"></label><span id="summary"></span><select id="group"></select><button id="prev">Previous</button><button id="next">Next</button></div><div class="panel"><h2 id="title"></h2><p id="status"></p><label>Notes (required if correction)<br><textarea id="notes"></textarea></label><p><button id="approve">All displayed pages belong to this submission</button> <button id="correct">Requires layout/scope correction</button></p><p id="message"></p></div><div id="images" class="images"></div>
