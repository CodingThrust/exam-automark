...
class MultiRouteReportingTests(unittest.TestCase):
    def test_builds_strict_aggregate_dashboard_and_typst(self):
        ...
        self.assertIn("Research question", typst)
        self.assertIn("Metric definitions", typst)
        self.assertIn("Snapshot SHA-256", typst)
        self.assertIn(SNAPSHOT, typst)
        self.assertIn('routes[*].run.packet_hash', typst)
        self.assertIn('routes[*].run.prompt_hash', typst)
        self.assertIn("render-multi-route-report --help", typst)
        self.assertIn("typst compile report.typ report.pdf", typst)
        self.assertIn("Limitations, safeguards, and operating rules", typst)
